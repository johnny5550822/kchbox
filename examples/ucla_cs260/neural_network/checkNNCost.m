% Check the costs and gradients for the NN and see if the backprop work
% perfectly

function [] = checkNNCost()

% Setup random data / small model
inputSize = 4;
hiddenSize = 5; %last layer
hiddenLayersSize = [3 5];
lambda = 0.01;
...
[cost, grad] = NNCost(stackedAETheta, inputSize, hiddenLayersSize, ...
                             numClasses, netconfig, ...
                             lambda, data, labels);

% Check that the numerical and analytic gradients are the same
numgrad = computeNumericalGradient( @(x) NNCost(x, inputSize, ...
                                        hiddenLayersSize, numClasses, netconfig, ...
                                        lambda, data, labels), ...
                                        stackedAETheta);
