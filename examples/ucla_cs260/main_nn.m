...
Debug = true;
if Debug
    hiddenLayersSizes = {[6 3]}; % numel(hiddenLayersSize) = number of hidden layers
    lambdas = [1e-4];         % weight decay parameter 
 
    % Create file to store result
    fileID = fopen('leave-one-out-result_dataset1/nn/result_nn.txt','w');
else    
    % Create file to store result
    fileID = fopen('leave-one-out-result/nn/result_nn.txt','w');
end
%% Step 1 Load data
...
for kk = 1:numel(lambdas)
    for tt = 1:numel(hiddenLayersSizes)   
        ...
        %save figure
        saveas(h_roc,sprintf('leave-one-out-result_dataset1/nn/nn_roc_exp%d.jpg',numExperiment));
